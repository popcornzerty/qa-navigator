...
Analyse a repository, review the generated User Stories and Gherkin, run the Playwright
suite, and measure QA coverage.

The repository holds both halves of the product:

| Path | What it is |
| --- | --- |
| `src/` | React + TypeScript frontend (Vite, TanStack Start, Tailwind, shadcn/ui) |
| `backend/` | **QA engine** — an independent FastAPI service (analysis, generation, automation, execution) |
| `tests/` | Playwright specs generated by the engine |

The two are only coupled by the REST contract: the frontend talks to the engine through
`src/api/`, and the engine can be deployed on its own. `backend/` is ignored by the
frontend build.
...
**2. QA engine** — in a second terminal:
...
pip install -e ".[dev]"
cp .env.example .env
uvicorn qa_engine.main:app --app-dir src --reload   # http://127.0.0.1:8000
```

`backend/.env` must list the frontend origin in `CORS_ORIGINS`
(`http://localhost:8080`), and the service is **restarted** for `.env` changes to take
effect — it does not reload them on the fly. `backend/README.md` documents the API,
the analysis pipeline, generation, Jira and configuration.

**3. Generation (optional)** — User Story and Gherkin generation runs on a local
[Ollama](https://ollama.com). Without it the analysis still completes; the two generation
steps simply report that they were skipped. Set `GENERATION_ENABLED=false` to turn them
off explicitly.

To work on the UI without any backend, set `VITE_API_MODE=mock` in `.env.local`: every
screen then runs on in-memory fixtures.

## Playwright

Generated specs land in `tests/` and run with this project's own config:

```bash
npx playwright test
```

`PLAYWRIGHT_BASE_URL` overrides the target origin; the config reuses a dev server that is
already listening instead of starting a second one.
